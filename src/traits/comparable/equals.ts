...
import { isIterable, isArrayLike, isObject, GenericObject } from '@benzed/types'
import { each } from '@benzed/each'
import Comparable from './comparable'

//// Helper ////

function equalArrayLikes(
...
function equals<T>(left: T, right: unknown): right is T {
    ...
    if (isLeftComparable) return left[Comparable.equals](right)

    const isRightComparable = Comparable.is(right)
    if (isRightComparable) return right[Comparable.equals](left)
    ...
    if (isArrayLike(left)) return equalArrayLikes(left, right as typeof left)
